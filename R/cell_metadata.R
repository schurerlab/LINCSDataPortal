...
{ xnew=toupper(cl)
  url <- httr::modify_url("http://dev3.ccs.miami.edu:8080", path = paste("/dcic/api/fetchentities?searchTerm=entityName:",xnew,"&fields=lincsidentifier,entityName,CL_LINCS_ID,CL_Alternative_Name,CL_Alternative_ID,CL_Organism,CL_Organ,CL_Disease,CL_Disease_Detail,IP_LINCS_ID,IP_Reprogramming_Method,IP_Passage_Last_Karyotyping,PC_LINCS_ID,PC_Donor_Age,PC_Provider_Name,DC_LINCS_ID",sep = ""))
  resp <- httr::GET(url)
