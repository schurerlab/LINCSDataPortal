...
{ 
  xnew <- toupper(pr)
  url <- httr::modify_url("http://dev3.ccs.miami.edu:8080", path = paste("/dcic/api/fetchentities?searchTerm=entityName:",xnew,"&fields=lincsidentifier,entityName,category,PR_Gene_Symbol,PR_Protein_Species,PR_UniProt_ID",sep = ""))
  resp <- httr::GET(url)
